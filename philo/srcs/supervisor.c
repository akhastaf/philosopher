...
/*   Updated: 2021/07/10 18:08:01 by akhastaf         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int     check_death(t_info *info)
{
    ...
    {
        if (!info->philos[i].is_eating && info->philos[i].last_time_eat)
        {
            pthread_mutex_lock(&info->philos[i].eating);
            cur_time = get_time();
            if (cur_time - info->philos[i].last_time_eat >= info->rules->time_to_die)
            {
                handle_message(&info->philos[i], DIED, KRED);
                return (1);
            }
